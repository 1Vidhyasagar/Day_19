var list = [
  ...
  function createlist({name,id,email}){
document.querySelector(".user_list").innerHTML += `<div class="list_container">
        <div>
           <h2 class="list_name">Name: ${name} </h2>
           <p class="list_id"> Id: ${id} </p>
           <p class="list_email">Email:${email} </p>
        <div>   
           </div>`
...
const noOfPages = Math.ceil(list.length/10);
...
for(let i =1;i<=noOfPages;i++){
  ...
  page.onclick=() =>{
    console.log("clicked the page button", i);
    const pageUsers = list.slice( (i-1)*10 ,i*10)
    document.querySelector(".user_list").innerHTML = "";
    pageUsers.forEach((object) => createlist(object));
}
  }
  const firstTenObj = list.slice(0,10);
  firstTenObj.forEach((object) => createlist(object));